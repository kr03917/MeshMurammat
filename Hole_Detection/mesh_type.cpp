...
Mesh Mesh_holes::generate_hole_highlight() {
	...
	Mesh::Property_map<halfedge_descriptor, unsigned char> red;
	bool red_created;
	boost::tie(red, red_created) = highlighted_mesh.add_property_map<halfedge_descriptor, unsigned char>("v:red", '255');
	assert(red_created);
	Mesh::Property_map<halfedge_descriptor, unsigned char> green;
	bool green_created;
	boost::tie(green, green_created) = highlighted_mesh.add_property_map<halfedge_descriptor, unsigned char>("v:green", '0');
	assert(green_created);
	Mesh::Property_map<halfedge_descriptor, unsigned char> blue;
	bool blue_created;
	boost::tie(blue, blue_created) = highlighted_mesh.add_property_map<halfedge_descriptor, unsigned char>("v:blue", '0');
	assert(blue_created);

	/*for (vertex_descriptor vd : highlighted_mesh.vertices()) {
		if (highlighted_mesh.is_border(vd)) {
...
bool Mesh_holes::hole_fill(Hole hole) {
	...
	while(true){
		...
		if (current_hole.vertices.size() == 4) {
			break;
		}
		if (angles.size() > 0) {
...
vertex_descriptor Mesh_holes::create_triangles_3(std::vector<vertex_descriptor> vertices, Angle* angle, Hole hole) {
	...
	if (is_new){
		...
		hole.patch_vertices.push_back(vd);
		...
		auto it = vertices.insert(vertices.begin(), vd);
	}
	else {
		angle->update = true;
	}
...
vertex_descriptor Mesh_holes::create_triangles_2(std::vector<vertex_descriptor> vertices, Angle* angle) {
	...
	if (is_new) {
		...
		//update angle
		//Angle* prev_angle = new Angle();
		//Angle* next_angle = new Angle();
